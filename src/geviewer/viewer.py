...
class GeViewer:
    ...
    def find_overlaps(self, tolerance=0.001, n_samples=100000, index=0, progress_obj=None):
        ...
        def find_overlaps_recursive(components, level=0, progress_obj=None):
            ...
            for comp in components:
                if comp['mesh'] is not None and not (comp['shape'] == 'Point' or comp['shape'] == 'Line'):
                    check_for_overlaps(comp, self.components[index]['children'], progress_obj)
                if len(comp['children']) > 0:
        ...
        if progress_obj:
            progress_obj.reset_progress()
            num_components = self.count_components(self.components[index]['children'], exclude_events=True, \
                                                   exclude_invisible=True)
            num_checks = int(num_components*(num_components - 1)/2)
            progress_obj.set_maximum_value(num_checks)

        find_overlaps_recursive(self.components[index]['children'], progress_obj=progress_obj)
